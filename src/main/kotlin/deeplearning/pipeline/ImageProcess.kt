...
class ImageProcess(
    ...
    fun getPrediction(): Triple<AppColors, String, File> {
        val highlightsAndObjects: Triple<File, List<DetectedObject>, DetectedPose> = detectAndHighlightObjects(selectedImage)
        val dominantColors: List<Color> = mainColorsInImage(selectedImage, 3)
        val generatedColorSchemeAndMessage: Pair<AppColors, String> = emotionClassify(
            dominantColors,
            highlightsAndObjects.second
        )

        return Triple(generatedColorSchemeAndMessage.first, generatedColorSchemeAndMessage.second, highlightsAndObjects.first)
    }

    private fun emotionClassify(
        colors: List<Color>,
        objects: List<DetectedObject>,
    ): Pair<AppColors, String> =
        predictEmotions(colors, objects).let {
            // This will return the found AppColor, along with the confidence and objects generated, etc.

            val predictedColorScheme: AppColors =
                when (it.predictedEmotion) {
